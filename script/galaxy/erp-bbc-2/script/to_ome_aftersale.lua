...
function gen_item_list(input)
    ...
    table.insert(list, tmp_sku)

    if input.gift_data == nil then

    else
        for k,v in pairs(input.gift_data) do
            gift = {
                num = v.gift_num,
                bn = v.bn
            }
...
function afterasle(input)
    local item_list = gen_item_list(input)
	local result = {
		...
		return_product_items 	= json.encode(item_list),
