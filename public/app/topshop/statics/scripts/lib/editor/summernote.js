...
(function (factory) {
  ...
  var EventHandler = function () {
    ...
    var insertImages = function (layoutInfo, files) {
      ...
      $.each(files, function (i, file) {
        editor.insertImage($editable, file);
      });
      // var callbacks = $editable.data('callbacks');
      ...
      //       data.append((options.uploadPerfix || 'upload_file') + i, file);
      //   });

      //   $.ajax({
      //       url: options.uploadURI + '',
      //       type: 'POST',
      //       data: data,
      //       cache: false,
      //       contentType: false,
      ...
      //                   editor.insertImage($editable, data.url, filenames[i]);
      //               });
      //           }
      //           else {
      //               if (callbacks.onImageUploadError) {
      //                   callbacks.onImageUploadError(rs);
      //               }
      //           }
      //       },
      //       error: function () {
  ...
  var Renderer = function () {
     ...
     * @param {String} [footer='']
     */
    var tplDialog = function (className, title, body, footer, isBig) {
      return '<div class="' + className + ' modal" aria-hidden="false">' +
               '<div class="modal-dialog' + (isBig ? ' modal-lg' : '') +'">' +
    ...
    var tplDialogInfo = {
      image: function (lang, options) {
        ...
        var body = '<input class="note-image-url form-control span12" type="hidden" />' + result;
        ...
        return tplDialog('note-image-dialog', '', body, '', 'true');
      },
  ...
  $.fn.extend({
    ...
    summernote: function (options) {
      ...
      options.langInfo = $.extend(true, {}, $.summernote.lang['zh-CN'], $.summernote.lang[options.lang]);
