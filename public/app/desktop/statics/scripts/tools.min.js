...
m=2;g.each(function(b){m+=b.offsetWidth+b.getPatch("margin").x});b.getElement("ul").setStyle("width",m);var l=new Fx.Scroll(k,{link:"cancel"}),n=function(){try{var c=a.offsetWidth;b[c<m?"addClass":"removeClass"]("tabs-scroll").setStyle("width",c);k.setStyle("width",c-2*k.getStyle("marginLeft").toInt());l.options.duration=500;l.scrollIntoView(b.getElement(".current"))}catch(f){}};n();LAYOUT.content_main.addEvent("resizelayout",n);c.addEvent("dispose",function(){LAYOUT.content_main.removeEvent("resizelayout",
