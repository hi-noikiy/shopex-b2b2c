(function(){
    ...
    imgDialog=new Class({
        ...
        imgcallback:function(image_id,image_src){
            ...
            this.onClose.call(this,window.returnValue);
            var ele = document.getElementsByClassName('scratchcard-bg-img');
            if(ele.length > 0) {
                var imgSrc = image_src.indexOf('_') > -1 ? image_src.substr(0, image_src.indexOf('_')) : image_src;
                $$(ele[0]).setStyle('background-image', 'url(' + imgSrc + ')');
            }
        }
    });
})();
