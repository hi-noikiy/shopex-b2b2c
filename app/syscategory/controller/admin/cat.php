...
class syscategory_ctl_admin_cat extends desktop_controller {
    ...
    public function edit($nCatId, $is_leaf=0)
    {
        ...
        $catInfo = $objMdlCat->getRow('cat_path, level, cat_name, cat_logo, cat_id, parent_id,  platform_fee, cat_service_rates, order_sort,cat_template', array('cat_id'=>$nCatId));

        // 组织上级分类数据，1级分类显示无，2级分类显示1级分类,3级分类则显示对应的2级和1级分类
        $catList[0] = array('cat_id'=>0, 'cat_name'=>app::get('syscategory')->_('----无----'), 'step'=>1);
        if($catInfo['level']=='2')
        {
            $level1CatInfo = $objMdlCat->getRow('cat_path, level, cat_name, cat_id, parent_id, platform_fee, cat_service_rates, order_sort', array('cat_id'=>$catInfo['parent_id']));
            $catList[1] = array('cat_id'=>$level1CatInfo['cat_id'], 'cat_name'=>$level1CatInfo['cat_name'], 'step'=>1);
        }
        if($catInfo['level']=='3')
        {
            $level2CatInfo = $objMdlCat->getRow('cat_path, level, cat_name, cat_id, parent_id, platform_fee, cat_service_rates, order_sort', array('cat_id'=>$catInfo['parent_id']));
            $level1CatInfo = $objMdlCat->getRow('cat_path, level, cat_name, cat_id, parent_id, platform_fee, cat_service_rates, order_sort', array('cat_id'=>$level2CatInfo['parent_id']));
            $catList[1] = array('cat_id'=>$level1CatInfo['cat_id'], 'cat_name'=>$level1CatInfo['cat_name'], 'step'=>1);
