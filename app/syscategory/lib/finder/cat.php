    ...
    public function column_control(&$colList, $list)
    {
        if($list)
        {
            ...
            foreach($list as $k=>$row)
            {
                ...
                $returnValue  = '<a href=\'?app=syscategory&ctl=admin_cat&act=edit&finder_id='.$_GET['_finder']['finder_id'].'&p[0]='.$catInfo['cat_id'].'&from_finder=1'.'\'" target="dialog::{title:\''.app::get('syscategory')->_('编辑分类').'\', width:550, height:450}">'.app::get('syscategory')->_('编辑').'</a>';
