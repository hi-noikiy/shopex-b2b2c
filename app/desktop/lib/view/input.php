...
class desktop_view_input
{
    function input_image($params)
    {
        ...
        $input_value_absolute = $params['absolute'] ? : 0;

        $image_src = base_storager::modifier($input_value);
        ...
        $html.= '<div><div class="image-input-view" style="font-size:12px;text-align:center;width:';
        ...
        $html.= '</div>';
        if($input_value_absolute){
            $html.= '<input type="hidden" name="'.$input_name.'" value="'.$image_src.'"/>';
        }else{
