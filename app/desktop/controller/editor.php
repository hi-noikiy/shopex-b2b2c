...
class desktop_ctl_editor extends desktop_controller{
    ...
    function uploader(){
        $html ='<form action="?ctl=editor&act=save_upload&name='.$_GET['name']
    ...
    function imglib(){
        ...
        return view::make('desktop/common/imglib.html', $this->pagedata);
    }

    function _img_list(){
        $img = $this->app->model('images');
        $html='';
        foreach($img->getList('url,s_url,l_url,m_url,id,width,height,storage',$filter,0,20) as $item){
            $url = '';
...
EOF;
    ...
    function table(){
        ...
        <div class="mainFoot"><div class="table-action">
            <button type="button" class="btn btn-default" id="mce_dlg_ok">'.app::get('desktop')->_('确定').'</button>
            <button type="button" class="btn btn-default" isclosedialogbtn="true">'.app::get('desktop')->_('取消').'</button>
        </div></div>
</div>
        <script>
...
#$html .=  '<input type="radio" name="type" value="goods" id="lnkToGoods"';
...
#$html.='>';
#$html.= '<label for="lnkToMail">'.app::get('desktop')->_('电子邮件').'</label>';
$html .= '</td>
        </tr>
    </table>
    <div id="dolink">
...
$html.=<<<EOF
        ><tr><th>$__a</th>
          <td><input type="text" size="20" id="iptGoodsFinder" /><button onclick="new Request.HTML({url:'?ctl=editor&act=find&p[0]=goods&p[1]='+encodeURIComponent($('iptGoodsFinder').value),'method':'get','update':'iptGoodsList'}).send()">$__b</button><br /><div id="iptGoodsList">{$goodsInfo}</div>
          </td>
        </tr>
    </table>
...
><tr>
  <th>$__c</th>
  <td><input type="text" size="20" id="iptArtFinder" /><button onclick="new Request.HTML({url:'?ctl=editor&act=find&p[0]=article&p[1]='+encodeURIComponent($('iptArtFinder').value),'method':'get','update':'iptArtList'}).send()">$__b</button><br />
...
><tr><th>'.app::get('desktop')->_('邮件地址:').'</th>
                ...
                <button type="button" class="btn btn-default" id="mce_dlg_ok"><span><span>'.app::get('desktop')->_('确定').'</span></span></button>
                <button type="button" class="btn btn-default" isclosedialogbtn="true"><span><span>'.app::get('desktop')->_('取消').'</span></span></button>
                </div></div>
';
        $html.="
<script>
...
";
     ...
     function image(){
        ...
        $html=<<<EOF
            ...
            <form id="imgFromSomeWhere" target="img-uploader" action="?ctl=editor&act=uploader" method="post" enctype="multipart/form-data">
            ...
            <div class="mainFoot"><div class="table-action">
            <button type="button" class="btn btn-default" id="mce_dlg_ok"><span><span>$__aq</span></span></button>
            <button type="button" class="btn btn-default" isclosedialogbtn="true"><span><span>$__aw</span></span></button>
            </div></div>
EOF;
        $html.=<<<EOF
        <script>
            ...
            function showResLib(tag_id,page_id){
              ...
              W.page('?ctl=editor&act=gallery&p[0]={0}&p[1]={1}'.format(tag_id,page_id),{method:'get',update:$('imgViewLibBox')});
            }
            </script>
EOF;
        echo $html;
    ...
    function find($type,$keywords){
        ...
        if(count($list)>0){
            ...
            return view::make('desktop/editor/dlg_result.html', $this->pagedata);
        }else{
            return app::get('desktop')->_('没有符合条件"')."<b>".$keywords."<b>".app::get('desktop')->_('"的记录。');
        }
    }
    ...
    function lista(){
        ...
        return view::make('desktop/editor/object_items.html', $this->pagedata);
    }
    ...
    function object_rows(){
        if($_POST['data']){
            ...
            return view::make('desktop/finder/input-row.html', $this->pagedata);
        }
    }
    function finder_common(){
        ...
        return $this->finder($_GET['app_id'].'_mdl_'.$_GET['object'],$params);
    }
    function selectobj(){
        $filter = $_GET['filter'];
        ...
        return view::make('desktop/editor/object_selector.html', $this->pagedata);
    }
