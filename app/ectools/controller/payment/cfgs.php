...
class ectools_ctl_payment_cfgs extends desktop_controller{
    ...
    function index(){
        return $this->finder('ectools_mdl_payment_cfgs',array(
            ...
            'actions'=>array(
               array(
                    'label'=>app::get('ectools')->_('设置货到付款'),
                    'href'=>'?app=ectools&ctl=payment_cfgs&act=toSetOffline',
                    'target'=>'dialog::{title:\''.app::get('ectools')->_('设置货到付款').'\',  width:500,height:320}',
                ),
            ),
        )
    );
    ...
    {
        $pagedata['open'] = app::get('ectools')->getConf('ectools.payment.offline.open');
        return view::make('ectools/payments/cfgs/offline.html',$pagedata);
    }

    public function doSetOffline()
    {
        $this->begin('?app=ectools&ctl=payment_cfgs&act=index');
        $postdata = utils::_filter_input(input::get('open'));
        $this->adminlog("设置是否启用货到付款[启用状态:{$postdata}]", 1);
        $result = app::get('ectools')->setConf('ectools.payment.offline.open',$postdata);
        $this->end($result);
    }
    ...
    public function _views(){
        ...
        $show_menu = array(
            ...
            4=>array('label'=>app::get('ectools')->_('应用端'),'optional'=>false,'addon'=>$app_num,    'filter'=>$app_filter),
        );
        return $show_menu;
    }
    ...
    function setting($pkey){
        ...
        if ($_POST['setting'])
        {
            ...
            //保存默认支付方式
            if($_POST['def_payment']){
                $arrServicelist = kernel::servicelist('ectools_payment.ectools_mdl_payment_cfgs');
                foreach($arrServicelist as $class_name => $object){
                    $strPaymnet = unserialize($this->app->getConf($class_name));
                    if(!empty($strPaymnet) && $payment->platform == 'iscommon'){
                        $strPaymnet['def_payment'] = 0;
                    }elseif(!empty($strPaymnet) && $strPaymnet['platform'] == $payment->platform || $strPaymnet['platform'] == 'iscommon'){
                        $strPaymnet['def_payment'] = 0;
                    }

                    $this->app->setConf($class_name,serialize($strPaymnet));
                }
            }
            ...
            $data['def_payment'] = $_POST['def_payment'];
            $data['pay_type'] = $_POST['pay_type'];
            $data['platform'] = $payment->platform;//支付平台
