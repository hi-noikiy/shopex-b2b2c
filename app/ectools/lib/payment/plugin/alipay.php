...
final class ectools_payment_plugin_alipay extends ectools_payment_app implements ectools_interface_payment_app {
    ...
    public function __construct($app)
    {
        ...
        // 退款异步返回地址
        $this->notify_url_refund = kernel::openapi_url('openapi.ectools_payment/parse/' . $this->app->app_id . '/ectools_payment_plugin_alipay', 'refundcallback');
    ...
    public function dopay($payment)
    {
        ...
        $this->add_field('subject',        $payment['item_title'].'...');
        ...
        $this->add_field('body', $payment['item_title'].'...');
        $this->add_field('sign', $this->md5Sign($this->fields, $mer_key));
        $this->add_field('sign_type','MD5'); //'MD5'要大写
    ...
    {
        $sHtml = "<form id='applyForm' target='_blank' name='applyForm' method='".$this->submit_method."' action='" . $this->submit_url . "'>";
    ...
    /**
     * 检验返回数据合法性
     * @param mixed $para 包含签名数据的数组
     * @param mixed $key 签名用到的私钥
     * @access private
     * @return boolean
     */
    public function is_return_vaild($para, $mer_key)
    {
        $sign = $this->md5Sign($para, $mer_key);
        ...
        #记录返回失败的情况
        logger::error(app::get('ectools')->_('支付宝失败支付单号：') . $para['out_trade_no'] . app::get('ectools')->_('签名验证不通过，请确认！')."\n");
        logger::error(app::get('ectools')->_('本地产生的加密串：') . $signstr);
        logger::error(app::get('ectools')->_('支付宝传递打过来的签名串：') . $para['sign']);
        $str_xml .= "<alipayform>";
...
/* 以下为退款代码 ↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓ */
    ...
    public function dorefund($payment)
    {
        ...
        logger::info('请求alipay参数：'.var_export($this->fields,1));

        // Generate html and send payment.
        // echo $this->gen_form();exit;
        $submit_html = $this->gen_form();
