 ...
 */
final class ectools_payment_plugin_teegonchinapayb2c extends ectools_payment_app {

    /**
     * @var string 支付方式名称
     */
    public $name = '天工收银（银联网关支付）';
    /**
     * @var string 支付方式接口名称
     */
    public $app_name = '天工收银（银联网关支付）';
    /**
     * @var string 支付方式key
     */
    public $app_key = 'teegonchinapayb2c';
    /**
     * @var string 中心化统一的key
     */
    public $app_rpc_key = 'teegonchinapayb2c';
    /**
     * @var string 统一显示的名称
     */
    public $display_name = '天工收银（银联网关支付）';
    /**
     * @var string 货币名称
     */
    public $curname = 'CNY';
    ...
    public function __construct($app)
    {
        ...
        $this->callback_url = kernel::openapi_url('openapi.ectools_payment/parse/' . $this->app->app_id . '/ectools_payment_plugin_teegonchinapayb2c', 'callback');
    ...
    function callback(&$in){
        $ikey = $this->getConf('PrivateKey', __CLASS__);
        $ret = array();
        $ret['payment_id'] = $in['order_no'];
        $ret['currency'] = 'CNY';
        $ret['money'] = $in['amount'];
        $ret['cur_money'] = $in['amount'];
        $ret['trade_no'] = $in['charge_id'];
        $ret['pay_app_id'] = "teegonchinapayb2c";
        $ret['pay_type'] = 'online';
        $ret['memo'] = 'teegonchinapayb2c';
        $ret['t_payed'] =$in['pay_time'];
        if(!$this->get_sign_veryfy($in,$in['sign'])){
            $ret['status'] = 'succ';
        }else{
