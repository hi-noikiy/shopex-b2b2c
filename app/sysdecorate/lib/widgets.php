...
class sysdecorate_widgets {
    ...
    public function config()
    {
        ...
        $config['wap']['params']['wapimageslider']['title'] = '图片广告页配置(不支持APP)';
    ...
    public function setParams($widgetsId, $widgetsName, $dialogName, $params, $shopId)
    {
        ...
        $rs = $this->objMdlWidgetsInstance->save($data);
        // 将店铺配置放到缓存中
        if($rs)
        {
            $this->_setWidgetsCache($shopId);
        }
    ...
    private function getParams($widgetsName, $shopId)
    {
        ...
        // 先获取缓存中的挂件配置，如果不存在，再从数据库中取
        if($tmpParams = $this->_getWidgetsCache($shopId))
        {
            $tmpArr = array();
            $tmpParams = unserialize($tmpParams);
            foreach ($tmpParams as $val)
            {
                if($val['widgets_type'] == $widgetsName && $val['shop_id'] == $shopId)
                {
                    $tmpArr[] = $val;
                }
            }
    ...
    // 设置挂件缓存key
    private function _setWidgetsCacheKey($shopId)
    {
        $key = 'WIDGET-SHOP';
        $key = $key.'-'.$shopId;
    ...
    // 设置挂件缓存
    private function _setWidgetsCache($shopId)
    {
        $key = $this->_setWidgetsCacheKey($shopId);
        // 设置缓存
        $params = $this->objMdlWidgetsInstance->getList('widgets_id,shop_id,widgets_type,params', array('shop_id'=>$shopId));
        if($params)
        {
            cache::store('sysdecorate')->put($key, serialize($params), 0);
        }
    ...
    // 获取挂件缓存
    private function _getWidgetsCache($shopId)
    {
        $key = $this->_setWidgetsCacheKey($shopId);
        // 获取缓存
        $result = cache::store('sysdecorate')->get($key);
