        ...
        if( $data['trade_rates'] )
        {
            ...
            foreach( $result['list'] as $k=>$row)
            {
                ...
                if( $row['rate_pic'] )
                {
                    ...
                    {
                        $ratePic = base_storager::modifier($ratePic);
                        $result['list'][$k]['rate_pic_t'] = base_storager::modifier($ratePic,'t');
                    }
                }
                ...
                if( $row['append'] )
                {
                    if( $row['append']['append_rate_pic'] )
                    {
                        ...
                        {
                            $appendRatePic = base_storager::modifier($appendRatePic);
                            $result['list'][$k]['append']['append_rate_pic_t'] = base_storager::modifier($ratePic,'t');
                        }
                    }
                }
                else
                ...
                $result['list'][$k]['is_check_append'] = 0;//默认不能进行追评
                ...
                if( ! $row['is_append'] && round((time() - $row['created_time'])/86400) < $day )
                {
                    ...
                    $result['list'][$k]['is_check_append'] = 1;
                }
            }
