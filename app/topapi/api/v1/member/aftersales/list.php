...
class topapi_api_v1_member_aftersales_list implements topapi_interface_api {
    ...
    public function handle($params)
    {
        ...
        foreach( $result['list'] as $key=>&$row )
        {
            $row['gift_count'] = 0;
            $totalItem = 0;
            if(isset($row['gift_data']) && $row['gift_data'])
            {
                $row['gift_data'] = $this->__preGiftData($row['gift_data']);
                $row['gift_count'] += array_sum(array_column($row['gift_data'],'gift_num'));
            }

            $totalItem += ($row['num']+$row['gift_count']);
            ...
            //如果当前子订单为判断过是否可以投诉 最新投诉的数据在最前，所以如果有重复投诉，后续则不需要在判断是否需要投诉
            if( ! isset($isComplaints[$row['oid']]) )
            {
                $isComplaints[$row['oid']] = true;
