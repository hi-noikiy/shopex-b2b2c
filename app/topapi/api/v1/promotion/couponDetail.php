...
class topapi_api_v1_promotion_couponDetail implements topapi_interface_api{
    ...
    public function handle($params)
    {
        ...
        $apiFilter = array(
            ...
            'orderBy' => $params['orderBy'] ? : ' item_id DESC',
            'fields' =>'item_id,shop_id,title,image_default_id,price,promotion_tag',
        );
        $apiFilter['coupon_id'] = $params['coupon_id'];
        $couponItem = app::get('topapi')->rpcCall('promotion.couponitem.list', $apiFilter);
        if(time() > $couponItem['promotionInfo']['canuse_end_time'])
        {
            throw new \LogicException(app::get('topapi')->_('优惠券不存在或者已经失效'), 30001);
        }

        if( $couponItem['list'] )
        {
            foreach( $couponItem['list'] as &$row  )
            {
                $row['image_default_id'] = base_storager::modifier($row['image_default_id']);
            }
        }
