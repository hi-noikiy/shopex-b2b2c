...
class topapi_api_v1_promotion_activityDetail implements topapi_interface_api{
    ...
    public function handle($params)
    {
        ...
        if(time()<$pagedata['info']['start_time'])
        {
            $pagedata['info']['status'] = 'comming';
        }
        if(time()>$pagedata['info']['start_time'] && time()<$pagedata['info']['end_time'])
        {
            $pagedata['info']['status'] = 'active';
        }
        // 处理本地远程链接转换
        $pagedata['info']['slide_images'] = base_storager::modifier($pagedata['info']['slide_images']);
        // 获取活动的商品列表
        ...
        foreach ($pagedata['list'] as &$v) {
            $v['item_default_image'] = base_storager::modifier($v['item_default_image']);
        }
        $pagedata['pagers']['total'] = $item['count'] ? : 0;

        // h5端链接
        $pagedata['share']['h5href'] = url::action('topwap_ctl_activity@detail', ['id'=>$params['activity_id']]);
        $pagedata['share']['image'] = base_storager::modifier($pagedata['info']['slide_images'], 't');
