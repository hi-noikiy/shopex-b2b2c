...
class topapi_api_v1_item_itemSearch implements topapi_interface_api{
    ...
    {
        $return = array(
            'item_id' => ['type'=>'string','valid'=>'','desc'=>'商品id，多个id用，隔开','example'=>'2,3,5,6'],
            'shop_id' => ['type'=>'int','valid'=>'integer','example'=>'','desc'=>'店铺id'],
            ...
            'search_shop_cat_id' => ['type'=>'int','valid'=>'','example'=>'','desc'=>'店铺搜索自有一级类目id'],
            'cat_id' => ['type'=>'int','valid'=>'','example'=>'1','desc'=>'平台的商品类目id','msg'=>'平台类目id必须是正整数'],
            'virtual_cat_id' => ['type'=>'int','valid'=>'','example'=>'1','desc'=>'平台的虚拟类目id','msg'=>'虚拟类目id必须是正整数'],
            ...
            'is_selfshop' => ['type'=>'bool','valid'=>'','example'=>'1','desc'=>'是否是自营 (1 自营)'],
    ...
    public function handle($params)
    {
        ...
        if($params['virtual_cat_id']){
            $virtualInfo = app::get('topapi')->rpcCall('category.virtualcat.info',['virtual_cat_id'=>intval($params['virtual_cat_id']),'platform'=>'app']);
            $initfilter = unserialize($virtualInfo['filter']);
            $initfilter['virtual_cat_id'] = $params['virtual_cat_id'];

            if($initfilter['brand_id'] && is_array($initfilter['brand_id']))
            {
                $initfilter['brand_id'] = implode(',', $initfilter['brand_id']);
            }
        }
        if($initfilter){
            $filter = array_merge($initfilter,$params);
        }
