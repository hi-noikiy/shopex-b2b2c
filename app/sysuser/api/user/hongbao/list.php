...
final class sysuser_api_user_hongbao_list {
    ...
    public function getParams()
    {
        $return['params'] = array(
            ...
            'used_platform' => ['type'=>'string', 'valid'=>'in:pc,wap,app', 'desc'=>'红包支持使用平台 pc wap 没有限制则不需要传入参数'],
    ...
    public function get($params)
    {
        ...
        if( $params['used_platform'] )
        {
            ...
            elseif( $params['used_platform'] == 'wap' )
            {
                $filter['used_platform|in'] = array('wap','all');
            }
            elseif( $params['used_platform'] == 'app' )
            {
                $filter['used_platform|in'] = array('app','all');
            }
        }
