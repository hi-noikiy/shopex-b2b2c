...
class sysrate_traderate {
     ...
     */
    public function add($data, $isShopDsr=true)
    {
        $params['tid'] = $data['tid'];
        $params['fields'] = 'tid,shop_id,user_id,status,end_time,orders.title,orders.end_time,orders.status,orders.spec_nature_info,orders.end_time,orders.price,orders.pic_path,orders.buyer_rate,orders.user_id,orders.aftersales_status,orders.shop_id,orders.oid,orders.anony,orders.item_id';
    ...
    public function update($rateId, $data)
    {
        ...
        $rowData = $this->objMdlTraderate->getRow('result,item_id', ['rate_id'=>$rateId]);
        if( empty($rowData) )
        {
            throw new \LogicException(app::get('sysrate')->_('修改的评价不存在'));
        }

        //更新商品评价数量统计
        $filter['item_id'] = $rowData['item_id'];
