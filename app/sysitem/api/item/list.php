...
class sysitem_api_item_list {
    ...
    public function getParams()
    {
        ...
        $return['extendsFields'] = ['item_store','item_status','sku', 'sku_id'];
    ...
    public function get($params)
    {
        ...
        if( $extends['sku'] )
        {
            ...
            $storeList = kernel::single('sysitem_item_redisStore')->getSkuStore($skuIds);
            if( $storeList )
            {
                foreach( $itemListData as $itemId=>$itemData )
                {
                    foreach( $itemData['sku'] as $skuId => $row )
                    {
                        $itemListData[$itemId]['sku'][$skuId]['store'] = $storeList[$skuId]['store'];
                        $itemListData[$itemId]['sku'][$skuId]['freez'] = $storeList[$skuId]['freez'];
                    }
                }
            }
        }
