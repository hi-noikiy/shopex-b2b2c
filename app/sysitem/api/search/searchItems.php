...
class sysitem_api_search_searchItems{
    ...
    public function getParams()
    {
        $return['params'] = array(
            ...
            'shop_id' => ['type'=>'int','valid'=>'','description'=>'店铺id,多个用逗号隔开','example'=>'','default'=>''],
    ...
    {
        $filterCols = ['item_id','shop_id','shop_cat_id','search_keywords','approve_status','brand_id','init_brand_id','prop_index','is_selfshop','dlytmpl_id'];
        foreach( $filterCols as $col )
        {
            if( $params[$col] )
            {
                ...
                if( in_array($col,['item_id','brand_id','init_brand_id','shop_cat_id','prop_index','shop_id']) )
                {
                    if( $col == 'shop_cat_id')
                    {
        ...
        if($params['search_shop_cat_id'] >0 && $params['shop_id']){
            ...
            $catIds = app::get('sysitem')->rpcCall('shop.cat.get', $catParams);
        ...
        if( $filter['init_brand_id'] && !$filter['brand_id'])
        {
            $filter['brand_id'] = $filter['init_brand_id'];
            unset($filter['init_brand_id']);
        }
    ...
    public function getList($params)
    {
        ...
        if(!$params['orderBy'])
        {
            $orderBy = "default_weight desc, item_id desc";
        }
