...
class sysitem_search_item  {
    ...
    public function __construct()
    {
        ...
        $this->objMdlItemSearchweight = app::get('sysitem')->model('item_search_weight');
        ...
        $this->itemSearchWeightTable = $this->objMdlItemSearchweight->table_name(1);
    }
    ...
    public function colsByTable($cols)
    {
        ...
        elseif( $this->objMdlItemSearchweight->schema['columns'][$cols] )
        {
            return $this->itemSearchWeightTable;
        }
    }
    ...
    public function getList($cols='*', $filter=array(), $offset=0, $limit=-1, $orderBy=null, $groupBy='')
    {
        ...
        $itemTableSearchWeightAlias = $qb->getConnection()->quoteIdentifier($this->itemSearchWeightTable);
        ...
        $qb->select($columns)
            ...
            ->leftJoin($itemTableAlias, $this->itemSearchWeightTable, $itemTableSearchWeightAlias, "{$itemTableAlias}.item_id={$itemTableSearchWeightAlias}.item_id")
            ->where($whereSql);
