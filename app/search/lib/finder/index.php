...
class search_finder_index
{
    ...
    public function column_edit(&$colList, $list)
    {
        ...
        {
            $colList[$k] = '<a href="?app=search&ctl=index&act=setting&_finder[finder_id]=' . $_GET['_finder']['finder_id'] . '&p[0]=' . $row['index_name'] . '" target="dialog::{title:\'' . app::get('search')->_('配置索引搜索参数') . '\', width:680, height:325}">' . app::get('search')->_('配置') . '</a>';
        }
    }
    ...
    public function detail_capability($row)
    {
        ...
        if($status){
            ...
            $tablesInfo = $obj->getDescribe($row, true);
