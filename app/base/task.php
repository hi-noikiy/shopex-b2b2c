...
class base_task
{
    public function install_options(){
        return array(
            ...
            'redisdbname'=>array('type'=>'select','vtype'=>'required','required'=>true,'default'=>'0','title'=>app::get('base')->_('redis数据库'),'options_callback'=>array('app'=>'base', 'method'=>'redisDatabase'),'onfocus'=>'setuptools.getdata(\'base\', \'redisDatabase\', this);'),
    ...
    public function pre_install($options){
        ...
        if (!$options['config-install']) {
            ...
            $options['redis.connection'] .= "?".$redisDbname;
    ...
    public function redisDatabase($options)
    {
        $options = $options['base'];
        $redisClient = new RedisClient($options['redis.connection'], $options);
        $database = $redisClient->config('GET','databases');
        $num = intval($database['databases']);
        $redisDatabase = [];
        for($i=0;$i<$num;$i++)
        {
            $redisDatabase['database='.$i] = 'database='.$i;
        }
