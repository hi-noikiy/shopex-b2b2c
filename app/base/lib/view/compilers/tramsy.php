...
class base_view_compilers_tramsy extends base_view_compilers_compiler implements base_view_compilers_interface
{
	 ...
	 * @return void
	 */
    public function setViewHelper($method,$helper)
    {
    ...
    {
        $pattern = sprintf('!%s\*.*?\*%s!su', $this->leftDelimiter, $this->rightDelimiter);
...
?>';
    ...
    protected function parseEnvVar($varStr)
    {
        ...
        {
            case 'CONFIG':
                $var_ns = 'config::get(\''.implode('.', array_slice($vars, 2)).'\')';
                $varStr = '';
                break;
            case 'CONF':
                ...
                break;
            case 'APP':
                // 第三个参数用为app_id, 第四个参数以后才能返回正确的值
                if( $forth )
                {
                    $var_ns = sprintf('app::get(\'%s\')->%s', $third, $forth);
                }
                else
                {
                    throw new \ErrorException('$env app\'s argument is wrong:('.$varStr.') ');
                }
                $varStr = '';
                break;
