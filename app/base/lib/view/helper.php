...
EOF;
    ...
    function modifier_hidden_show($string, $etc='*', $encoding='utf-8')
    {
        //$first = $string[0];
        $first = mb_substr($string, 0, 1, $encoding);
        $last = mb_substr($string, -1, 1, $encoding);
        for( $i=0; $i<4; $i++ )
        {
            $hiddenStr .= $etc;
        }
    ...
    //防止重复提交和csrf
    function modifier_gettocken($status)
    {
        $sess_id = kernel::single('base_session')->sess_id();
        $time = time();
        $_SESSION['rsftocken'] = $time.'-'.$sess_id;
        return $_SESSION['rsftocken'];
    }
