...
class base_validator_validator
{
    ...
    public function __construct( array $data, array $rules, array $messages = array())
    {
        ...
        $this->translator = new base_validator_translator();
    }
    /**
    ...
    {
        $this->messages = new base_validator_messageBag;
     ...
     * @return bool
     */
    public function newFails()
    {
        if( ! $this->passes() )
        {
            $messages = $this->messagesInfo();
     ...
     * @return void
     */
    protected function validate($attribute, $rule,$describe=null)
    {
        ...
        {
            $this->addFailure($attribute, $rule, $parameters,$describe);
        }
    }
     ...
     * @return void
     */
    protected function addFailure($attribute, $rule, $parameters,$describe)
    {
        $this->addError($attribute, $rule, $parameters,$describe);
     ...
     * @return void
     */
    protected function addError($attribute, $rule, $parameters,$describe)
    {
        $message = $this->getMessage($attribute, $rule);

        $message = $this->doReplacements($message, $attribute, $rule, $parameters,$describe);
     ...
     * @return bool
     */
    protected function validateSthan($attribute, $value, $parameters)
    {
        $this->requireParameterCount(1, $parameters, 'different');
     ...
     * @return bool
     */
    protected function validateBthan($attribute, $value, $parameters)
    {
        $this->requireParameterCount(1, $parameters, 'different');
     ...
     * @return bool
     */
    protected function validateThan($attribute, $value, $parameters)
    {
        $this->requireParameterCount(1, $parameters, 'different');
    ...
    /**
     * Validate that an attribute is a mobile.
     *
     * @param  string  $attribute
     * @param  mobile   $value
     * @return bool
    ...
    {
        return !preg_match('/[^\x00-\x80]/', $value);

    }

    /**
     * Validate that an attribute is a loginAccount.
     *
     * @param  string  $attribute
     * @param  mobile   $value
     * @return bool
     */
    protected function validateLoginaccount($attribute, $value)
    {
        return !preg_match('/^\d*$|@/', $value);
    }
    /**
     * Validate that an attribute is a tel.
     *
     * @param  string  $attribute
     * @param  tel   $value
     * @return bool
     */
    protected function validateTel($attribute, $value)
    {

        return preg_match("/^(0?(([1-9]\d)|([3-9]\d{2}))-?)?\d{7,8}$/",$value);
    }
    ...
    protected function getMessage($attribute, $rule)
    {
        ...
        //echo '<pre>';print_r($inlineMessage);exit();
     ...
     * @return string
     */
    protected function doReplacements($message, $attribute, $rule, $parameters,$describe)
    {
        if(empty($describe))
        {
