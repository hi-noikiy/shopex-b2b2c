...
use base_http_request as Request;

class base_routing_route
{
	/**
	 ...
	 * Run the route action and return the response.
	 *
	 * @param  base_http_request  $request
	 * @return mixed
	 */
	protected function runCallable(Request $request)
	{
        ...
        logger::emergency('aaaa_---', $this->action['uses']);
		return call_user_func_array($this->action['uses'], $parameters);
	}
	...
	public static function getValidators()
	{
		...
		return static::$validators = array(
			new base_routing_matching_method, new base_routing_matching_scheme,
			new base_routing_matching_host, new base_routing_matching_uri,
		);
	}
