...
class base_rpc_server
{
    ...
    //  print_r(input::header('X-api-arg'));exit;
        ...
        try{
            ...
            //是否需要强制使用API定义的参数
            if( $class->use_strict_filter )
            {
                //验证数据
                //通过传入数据和api原定义的类型进行比对
                $realApiParams = ecos_validatorApiParams($paramsInfos['params'], $params);

                //验证json结构的参数
                //$realApiParams 为API传入参数验证后API定义的参数
                //以前的API有些在API中未定义确使用了，因此该API参数为兼容暂时不使用
                $realApiParams = ecos_validatorApiJsonParams($paramsInfos['params'], $params, $realApiParams);
                $params = apiUtil::pretreatment($realApiParams, $paramsInfos);
            }
            else
            {
        ...
        catch(LogicException $e)
        {
            ...
            return $this->__sendError($method.".logicException", $errorMessage, $format, $exceptionClass);
        }
        catch(Exception $e)
        {
