...
class system_messenger_smschg{
    ...
    public function send($contents,$config) {
        ...
        if($result['res']=='succ') {
            ...
            $msg = app::get('system')->_('无返回状态，短信发送失败！');
            // start-更新发送短信日志状态到表
            $smsUpdateLog = ['sms_id'=>$smsLog['sms_id'], 'status'=>'fail', 'msg'=>$msg];
            app::get('system')->model('smslog')->save($smsUpdateLog);
            // end
    ...
    public function newSend($contents,$config)
    {

        $result = $this->__newsend($contents,$config);
        ...
        if($result['res']=='succ') {
            ...
            if($result['msg'] == "10005")
            {
                if(kernel::single('system_data_messenger')->setForeverToken())
                {
                    $this->newSend($contents,$config);
                }
            }
            throw new \LogicException($result['info']['error']);
            return false;
        }else{
    ...
    private function __newsend($contents,$config)
    {
        if(!$contents)
        {
            $msg=app::get('system')->_('手机短信不能为空！');
        ...
        $token = kernel::single('system_data_messenger')->getForeverToken($this->getEntId());

        $params = array(
            'shopexid' => $this->getEntId(),
            'source' =>$this->getSource($config['specialChannel']),
            'certi_app' => 'sms.newsend',
            'sendType' => $config['sendType'] ? $config['sendType'] : 'notice',
        );

        $params['contents'] = json_encode($contents);
        $params['token'] = $token;
        $params['certi_ac'] = $this->make_shopex_ac($params,$this->getSourceToken($config['specialChannel']));

        $result = prism::post('/smsv2/send',$params);
    ...
    public function getCerti() {
        ...
        if($this->certi){
            return $this->certi;
        }elseif(base_certi::get('certificate_id')){
            return  base_certi::get('certificate_id');
        }else{
            return false;
        }
    }
    ...
    public function getToken() {
        if($this->token){
            return $this->token;
        }elseif(base_certi::get('token')){
            return  base_certi::get('token');
        }else{
            return false;
        }
    }
