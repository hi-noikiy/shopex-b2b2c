...
final class syspromotion_api_hongbao_create {
    ...
    public function getParams()
    {
        $return['params'] = array(
            ...
            'used_platform'    => ['type'=>'string', 'valid'=>'required|in:all,pc,wap,app', 'title'=>'使用平台',    'desc'=>'使用平台(all,pc,wap,app)'],
            'total_money'      => ['type'=>'number', 'valid'=>'',                 'title'=>'红包总金额',        'desc'=>'可领取的红包总金额'],
            'user_total_money' => ['type'=>'number', 'valid'=>'required|numeric|min:0|max:1000000', 'title'=>'用户可领取总金额',  'desc'=>'用户可领取红包的总金额|必须为数字|最小必须大于0|最大不能超过1000000'],
            'user_total_num'   => ['type'=>'string', 'valid'=>'required|numeric|min:1|max:1000000', 'title'=>'用户可领取总数',    'desc'=>'用户可领取红包的总数量|必须为数字|最小必须大于1|最大不能超过1000000'],
    ...
    public function create($params)
    {
        ...
        foreach( $params['hongbao_list'] as $k=>$v)
        {
            ...
            if($v['hongbao_type'] == 'stochastic' && $v['money']*100 < $v['num'])
            {
                throw new \LogicException('红包数量必须小于等于金额*100');
            }
            ...
            elseif($params['hongbao_type'] == 'stochastic')
            {
                $totalMoney = ecmath::number_plus(array($totalMoney, $v['money']));
            }
