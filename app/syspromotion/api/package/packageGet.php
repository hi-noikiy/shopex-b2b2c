...
final class syspromotion_api_package_packageGet {
    ...
    {
        $objLibPromotionData = kernel::single('syspromotion_data_object')->setPromotion('package');
        $packageInfo = $objLibPromotionData->getPromoitonInfo($params['package_id']);
        $packageInfo['valid'] = $this->__checkValid($packageInfo);
        if($params['package_itemList'])
        {
            $packageItems = $objLibPromotionData->getPromtionItems($params['package_id']);
