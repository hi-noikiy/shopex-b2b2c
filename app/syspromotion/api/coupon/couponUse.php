...
final class syspromotion_api_coupon_couponUse {
    ...
    public function getParams()
    {
        $return['params'] = array(
            ...
            'user_id' => ['type'=>'string', 'valid'=>'', 'default'=>'', 'example'=>'', 'desc'=>'使用优惠券用户ID'],
        );
    ...
    {
        $filter['user_id'] = $params['oauth']['account_id'] ?: $params['user_id'];
        $filter['coupon_code'] = $params['coupon_code'];
        $filter['mode'] = $params['mode'];
        $filter['platform'] = in_array($params['platform'], array('pc', 'wap', 'app')) ? $params['platform'] : 'pc';
