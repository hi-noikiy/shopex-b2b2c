...
final class syspromotion_api_gift_giftGet{
	...
	{
        $objLibPromotionData = kernel::single('syspromotion_data_object')->setPromotion('gift');
        $giftInfo = $objLibPromotionData->getPromoitonInfo($params['gift_id']);
        $giftInfo['valid'] = $this->__checkValid($giftInfo);
        if($params['gift_itemList'])
        {
            $giftItems = $objLibPromotionData->getPromtionItems($params['gift_id']);
            ...
            $giftSkus = $objLibPromotionData->getgiftSku($params['gift_id']);
