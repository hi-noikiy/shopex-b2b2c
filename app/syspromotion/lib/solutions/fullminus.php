...
class syspromotion_solutions_fullminus implements syspromotion_interface_promotions{
    ...
    public function apply($params)
    {
        ...
        $applyNum = app::get('syspromotion')->rpcCall('trade.promotion.applynum', array('promotion_id'=>$params['promotion_id'], 'user_id'=>$params['user_id']));
        ...
        if( $params['forPromotionTotalPrice'] >=$ruleArray['full'][$ruleLength-1] )
        {
            if($fullminusInfo['canjoin_repeat']=='1')
            {
                ...
                if($ruleArray['full'][$ruleLength-1]<=0) return 0;
                $beishu = floor( bcdiv($params['forPromotionTotalPrice'], $ruleArray['full'][$ruleLength-1], 3) ) ;
