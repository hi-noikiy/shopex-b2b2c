...
class syspromotion_finder_activity_register{
    ...
    public function detail_basic($id)
    {
        ...
        // 获取活动规则信息
        $activityParams['activity_id'] = $registerData['activity_id'];
        $pagedata = app::get('syspromotion')->model('activity')->getRow('*',$activityParams);
        ...
        $pagedata['modified_time'] = $registerData['modified_time'];
        ...
        $registerItemList = app::get('syspromotion')->model('activity_item')->getList('*',$itemParams);
        foreach($registerItemList as &$value)
        {
            if(count($value['sku_activity_price']) < 1){
                $value['nospec'] = 0;
            }else{
                $value['nospec'] = 1;
            }
            $value['url'] = url::route('shopadmin', ['app'=>'syspromotion','act'=>'showSkuPrice','ctl'=>'admin_activity_register','itemId'=>$value['item_id'],'shopId'=>$value['shop_id'],'activityId'=>$value['activity_id']]);
        }
        $pagedata['itemsList'] = $registerItemList;
