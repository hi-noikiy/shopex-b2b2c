...
class topc_ctl_member_trade extends topc_ctl_member {
    ...
    public function tradeList()
    {
        ...
        $params = array(
            ...
            'fields' =>'order.spec_nature_info,tid,shop_id,user_id,status,cancel_status,payment,points_fee,total_fee,post_fee,payed_fee,receiver_name,created_time,receiver_mobile,discount_fee,need_invoice,adjust_fee,order.title,order.price,order.num,order.pic_path,order.tid,order.oid,order.aftersales_status,buyer_rate,order.complaints_status,order.item_id,order.cat_id,order.shop_id,order.status,order.end_time,order.spec_nature_info,activity,pay_type,order.sendnum,order.gift_data',
        );
        ...
        foreach( $tradelist as $key=>&$row)
        {
            ...
            foreach( $row['order'] as $k=>$orderListData )
            {
                ...
                if($orderListData['cat_id'] && $orderListData['status'] =='TRADE_FINISHED' && $orderListData['end_time'])
                {
                    $aftersalesEnabled = app::get('topc')->rpcCall('aftersales.isEnabled',['oid'=>$orderListData['oid']]);
                    $tradelist[$key]['order'][$k]['refund_enabled'] = $aftersalesEnabled['refund_enabled'];
                    $tradelist[$key]['order'][$k]['changing_enabled'] = $aftersalesEnabled['changing_enabled'];
                }
            }
            // 获取店铺子域名
