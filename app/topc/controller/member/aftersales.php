    ...
    public function aftersalesApply()
    {
        ...
        try
        {
            $tradeFiltr = array(
                'tid' => $tid,
                'oid' => $oid,
                'fields' => 'tid,user_id,shop_id,status,receiver_state,receiver_city,receiver_district,receiver_name,receiver_mobile,receiver_address,created_time,orders.oid,orders.user_id,orders.sku_id,orders.num,orders.sendnum,orders.title,orders.pic_path,orders.price,orders.payment,orders.cat_id,orders.end_time,orders.status,orders.complaints_status,orders.aftersales_status,orders.gift_data',
            );
            $pagedata['tradeInfo']= app::get('topc')->rpcCall('trade.get', $tradeFiltr,'buyer');

            foreach($pagedata['tradeInfo']['orders'] as $k => $order)
            {
                if($order['cat_id'] && $order['status'] =='TRADE_FINISHED' && $order['end_time'])
                {
                    $aftersalesEnabled = app::get('topc')->rpcCall('aftersales.isEnabled',['oid'=>$oid]);
                    $pagedata['refund_enabled'] = $aftersalesEnabled['refund_enabled'];
                    $pagedata['changing_enabled'] = $aftersalesEnabled['changing_enabled'];
                }
