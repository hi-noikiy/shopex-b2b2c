...
class topc_ctl_member_rate extends topc_ctl_member {
   ...
   /*
    ...
    public function createRate()
    {
        ...
        //获取该订单的店铺类型信息
        $shop = app::get('topc')->rpcCall('shop.get',array('shop_id'=>$tradeInfo['shop_id'], 'fields'=>'shop_type'));
        $tradeInfo['shop_type'] = $shop['shop_type'];
    ...
    public function doCreateRate()
    {
        ...
        foreach( input::get('rate_data') as $key=>$row )
        {
            ...
            {
                return $this->splash('error','',"中评和差评时，评价必填",true);
            }
    ...
    private function __getItemData()
    {
        ...
        foreach( $pagedata['rate'] as $k=>$row)
        {
            ...
            $pagedata['rate'][$k]['is_check_append'] = 0;//默认不能进行追评
            $day = (int)app::get('sysconf')->getConf('rate.append.time') ?: 30;
            if( ! $row['is_append'] && round((time() - $row['created_time'])/86400) < $day )
            {
                //如果没有追评，并且在追评时间限制之内，则可以追评
                $pagedata['rate'][$k]['is_check_append'] = 1;
            }
        }
     ...
     * 显示追评表单页面
     */
    public function showAppendRateView()
    {
        $params['rate_id'] = input::get('rate_id',false);
        $params['role'] = 'buyer';
        $params['fields'] = 'rate_id,created_time,tid,item_pic,item_id,item_title,item_price,result,content,rate_pic,is_append';
        $data = app::get('topc')->rpcCall('rate.get', $params,'buyer');

        $day = (int)app::get('sysconf')->getConf('rate.append.time') ?: 30;
        if( empty($data) || $data['is_append'] || round((time() - $data['created_time'])/86400) > $day )
        {
            redirect::action('topc_ctl_member_rate@index')->send();exit;
        }

        $pagedata['rateInfo'] = $data;
        $this->action_view = "rate/append.html";
        return $this->output($pagedata);
    }
    ...
    {
        $params['rate_id'] = input::get('rate_id');
        $params['content'] = trim(input::get('content'));
        $params['user_id'] = userAuth::id();

        $validator = validator::make(
            ['content'=>$params['content']],
            ['content'=>'required|min:5|max:300'],
            ['content'=>'追评内容必填|追评内容最少5个字|追评内容最多300个字']
        );
        if ( $validator->fails() )
        {
            $msg = $validator->messages()->first('content');
            return $this->splash('error',$url,$msg,true);
        }

        if( input::get('pic') )
        {
            $params['pic'] = implode(',',input::get('pic'));
        }

        try
        {
            $result = app::get('topc')->rpcCall('rate.append', $params);
        }
        catch(\LogicException $e)
        {
            $result = false;
