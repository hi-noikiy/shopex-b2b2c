...
class topc_ctl_cart extends topc_controller {
    ...
    public function checkout()
    {
        ...
        app::get('topc')->rpcCall('trade.cart.voucher.cancel', $apiParams);
    ...
    public function getVouchers()
    {
        ...
        $voucherListData = app::get('topc')->rpcCall('user.voucher.list.get', $params);
        if( $voucherListData['list'] )
        {
            foreach( $voucherListData['list'] as $k=>$voucherInfo)
            {
                ...
                if( $voucherTotalPrice >= $limitMoney  )
                {
                    ...
                    $pagedata['list'][$k] = $voucherInfo;
                }
            }
        }

        return  view::make('topc/checkout/vouchers.html', $pagedata)->render();
    }
    ...
    public function useVoucher()
    {
        try
        {
            ...
            if( app::get('topc')->rpcCall('trade.cart.voucher.add', $apiParams) )
            {
                $msg = app::get('topc')->_('使用购物券成功');
                return $this->splash('success', null, $msg, true);
            }
        }
        catch(Exception $e)
    ...
    public function useCoupon()
    {
        try
        {
            ...
            {
                //优惠券使用后清空购物券
                app::get('topc')->rpcCall('trade.cart.voucher.add', ['voucher_code' => -1, 'user_id' => userAuth::id(), 'platform' => 'pc' ]);
