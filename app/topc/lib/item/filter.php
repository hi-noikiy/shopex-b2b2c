...
class topc_item_filter {
    ...
    */
    ...
    public function decode( $params )
    {
        ...
        //orderby判断
        if(isset($params['r_sort']))
        {
            if(in_array($params['r_sort'], $this->orderBy))
            {
                $params['r_sort'] = $params['r_sort'];
            }
            else
            {
                $params['r_sort'] = $this->orderBy[0];
            }
        }
        //orderby判断
        if(isset($params['sort']))
        {
            if( in_array($params['sort'], $this->orderBy) )
            {
                $params['sort'] = $params['sort'];
            }
            else
            {
                $params['sort'] = $this->orderBy[0];
            }
        }
        switch( $params['operationtype'] )
        {
            ...
            case 'replace':
                //如果切换一级类目
                if( $params['r_lv1_cat_id'] )
                {
                    $replaceLv1Catid = true;
                }
