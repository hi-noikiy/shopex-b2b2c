...
class sysshop_passport {
     ...
     * @param $status 成功失败状态
     */
    protected final function sellerloginlog($memo)
    {
        if(!$this->shopId)
        {
            $shopId = app::get('sysshop')->rpcCall('shop.get.loginId',array('seller_id'=>pamAccount::getAccountId()),'seller');
        }
        else
        {
            $shopId = $this->shopId;
        ...
        return system_queue::instance()->publish('sysshop_tasks_sellerloginlog', 'sysshop_tasks_sellerloginlog', $queue_params);
    }
