...
class sysshop_data_shop {
     ...
     * @return bool
     */
    public function saveShop($shopData, $insertFlag)
    {
        ...
        {
            $msg = "该店铺名称已经存在，请重新设置店铺名称." . $shopData['shop_name'];
            throw new \LogicException($msg);
        }
        if($insertFlag == true){
            $result = $objMdlShop->insert($shopData);
            event::fire('shop.open', [$result]);
        }
        else
        {
            $result = $objMdlShop->save($shopData);
