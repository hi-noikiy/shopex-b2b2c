...
class topwap_ctl_shop extends topwap_controller{
    ...
    public function index()
    {
        ...
        $openNewdecorate = redis::scene('shopDecorate')->hget('wapdecorate_status','shop_'.$shopId);
        $apiParams = [
            'shop_id'    => $shopId,
            'page_name'  => 'index',
            'platform'   => 'wap',
        ...
        $newdecorateData = app::get('topshop')->rpcCall('sysdecorate.widgets.get', $apiParams);

        if($topshopNewSetup || ($openNewdecorate =='open' && $newdecorateData)){
            return redirect::action('topwap_ctl_newshop@index',['shop_id'=>$shopId]);
        }
