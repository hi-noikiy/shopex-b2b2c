...
class sysstat_data_shoptrade
{
	...
	public function getItemStaticsInfo($filter=array())
	{
		...
		$itemData = $itemStaticMdl->getList($rows, $rowsFilter);
		foreach ($itemData as $key => $value)
		{
			$itemInfo[$value['item_id']][] = $value;
		}
		...
		foreach ($itemInfo as $key => $value)
		{
			...
			{
				$result[$key]['shop_id'] = $v['shop_id'];
				$result[$key]['item_id'] = $v['item_id'];
				$result[$key]['title'] = $v['title'];
				$result[$key]['pic_path'] = $v['pic_path'];
				$result[$key]['amountnum'] += $v['amountnum'];
				$result[$key]['amountprice'] += $v['amountprice'];
				$result[$key]['refundnum'] += $v['refundnum'];
				$result[$key]['changingnum'] += $v['changingnum'];
			}
		}
		$order = explode(' ',$orderBy);
		$resultData = $this->arr_sort($result,$order[0],$order[1]);
		foreach ($resultData as $key => $value)
		{
			$itemOrder[]=$value;
		}
		return array_slice($itemOrder,$start,$limit);
	}
	...
	public function getItemCount($filter=array())
	{
		...
		//$itemStaticMdl = app::get('sysstat')->model('item_statics');
		$data['filter'] =  $filter;
		$itemData = $this->getItemStaticsInfo($data);
	...
	public function graphdata($type,$postdata,$compareData=null,$sendtype=null,$comparetime=null)
	{
		...
		if($compareData!=null)
		{
			...
			foreach ($postdata as $key => $value) {
				$compare[] = array(
						$value['createtime']* 1000 ,
						$compareinfo[$key][$type] ? $compareinfo[$key][$type] : 0,
					);
			}
	...
	public function tradegraphdata($type,$postdata,$sendtype=null,$filter=null)
	{
		...
		if($sendtype=='selecttime')
		{
			...
			foreach ($postdata['before'] as $key => $value)
			{
				$datas[] = array(
						$value['createtime']* 1000 ,
						$postdata['after'][$key][$type] ? $postdata['after'][$key][$type] : 0,
					);
			}
