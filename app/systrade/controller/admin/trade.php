...
class systrade_ctl_admin_trade extends desktop_controller{
    ...
    public function doCancel($tid)
    {
        ...
        $tradeInfo = kernel::single('systrade_getTradeData')->getTradeInfo(['rows'=>'shop_id'], $tid);
        $shopConf = app::get('topshop')->rpcCall('open.shop.develop.conf', ['shop_id'=>$tradeInfo['shop_id']]);
        if($shopConf['develop_mode'] == 'DEVELOP')
        {
            return view::make('systrade/permission.html', ['url'=>input::get('next_page', request::server('HTTP_REFERER'))]);
        }
        $pagedata['tid'] = $tid;
    ...
    public function cancelTrade()
    {
        ...
        $params['tid'] = $cancelData['tid'];
        $params['cancel_reason'] = $cancelData['cancel_reason'];
        $validator = validator::make([trim($params['cancel_reason'])],['required|max:50'],['取消原因必须填写!|取消原因最多填写50个字']);
        if ($validator->fails())
        {
        ...
        try
        {
            app::get('systrade')->rpcCall('trade.cancel.create',$params);
    ...
    public function _views()
    {
        ...
        $sub_menu = array(
            ...
            6=>array('label'=>app::get('systrade')->_('已结算'),'optional'=>false,'filter'=>array('settlement_status'=>1,'disabled'=>0)),
        );
        ...
        foreach($sub_menu as $k=>$v)
        {
            if($v['optional']==false)
            {
                ...
                if($k==$_GET['view'])
                {
                    ...
                    $show_menu[$k]['addon'] = $mdl_aftersales->count(array_merge($_POST,$v['filter']));
                }
