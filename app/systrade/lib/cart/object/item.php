...
class systrade_cart_object_item implements systrade_interface_cart_object{
    ...
    public function check($checkParams, $itemData, $skuData)
    {
        ...
        if($checkParams['is_checked'] && $validQuantity < $checkParams['totalQuantity'] )
        {
    ...
    public function checkObject($params, $basicCartData)
    {
        ...
        else
        {
            ...
            if($cartCount['variety'] >= 50 && $params['mode'] == 'cart')
            {
        ...
        $validQuantity = $skuData['store'] - $skuData['freez'];
        if($params['is_checked'] && $params['totalQuantity'] > $validQuantity)
        {
            throw new \LogicException(app::get('systrade')->_("库存不足, 最大库存为".$validQuantity));
            return false;
        }
    ...
    public function processCartObject($row,$itemsData,$skusData,$itemActivityList)
    {
        ...
        // 活动，剩余购买数量,如果剩余
        $activityBuyInfo = $itemActivityList[$itemId][$skuId];
        ...
        if($activityDetail && ((!$activityDetail['sku_id'] && !$activityDetail['sku_activity_price']) || $activityDetail['sku_activity_price'][$skuId]))
        {
            $objectData['price']['price'] = $activityDetail['sku_activity_price'][$skuId] ? $activityDetail['sku_activity_price'][$skuId] : $activityDetail['activity_price'] ; //购买促销后单价
            $objectData['price']['total_price'] = ecmath::number_multiple(array($objectData['price']['price'],$row['quantity'])); //购买此SKU总价格
