...
class systrade_data_complaints {
    ...
    {
        $complaintsData = $this->objMdlComplaints->getRow($fields, ['oid'=>$oid,'user_id'=>$userId], 'complaints_id desc');
     ...
     * @param $ordersFields 需要返回子订单数据的字段
     */
    public function getList($userId, $fields, $start, $limit, $orderBy='complaints_id desc')
    {
        $count = $this->objMdlComplaints->count(array('user_id' => $userId));
        if(! $count)
        {
            return false;
        }

        $list = $this->objMdlComplaints->getList($fields, array('user_id' => $userId), $start, $limit, $orderBy);
    ...
    // 目前投诉详情可以通过子订单号和投诉自增号进行查询
    public function getDetailInfo(array $filter, $userId, $fields, $ordersFields=null)
    {
        if(!$filter)
        {
            throw new \LogicException(app::get('systrade')->_('参数错误'));
        }
        $complaintsData = array();

        if($filter['oid'])
        {
            $complaintsData = $this->getInfo($filter['oid'], $userId, $fields, $ordersFields);
        }

        if($filter['complaints_id'])
        {
            $complaintsData = $this->objMdlComplaints->getRow($fields, ['complaints_id'=>$filter['complaints_id'],'user_id'=>$userId]);

            if( $ordersFields && $complaintsData )
            {
                $oid = $complaintsData['oid'];
                $complaintsData['orders'] = $this->objMdlOrder->getRow($ordersFields, ['oid'=>$oid]);
            }
