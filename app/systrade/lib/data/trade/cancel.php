...
class systrade_data_trade_cancel
{
    ...
    {
        $data = app::get('systrade')->model('trade_cancel')->getRow('shop_id,user_id,cancel_from,cancel_id,payed_fee', ['tid'=>$tid]);
    ...
    private function __backVoucher($tid)
    {
        $tradeInfo = app::get('systrade')->model('trade')->getRow('tid,user_id', array('tid'=>$tid));
        $userId = $tradeInfo['user_id'];
        $data = app::get('systrade')->rpcCall('user.voucher.list.get',['user_id'=>$userId,'tid'=>$tid,'fields'=>'tid,voucher_id,user_id,voucher_code']);
        if( !$data['list'] ) return true;

        //获取使用购物券的tid
        $useVoucherTid = explode(',',$data['list'][0]['tid']);
        $count = app::get('systrade')->model('trade_cancel')->count(['tid'=>$useVoucherTid]);
        $voucherCode = $data['list'][0]['voucher_code'];
        if( $count == count($useVoucherTid) )
        {
            app::get('systrade')->rpcCall('user.voucher.back',['voucher_code'=>$voucherCode,'user_id'=>$userId]);
        }
