...
class systrade_data_trade_create {
   ...
   /**
    ...
    public function generate($tradeParams, $aCart=array() )
    {
        ...
        if( $aCart['voucher']['voucher_id'] )
        {
            $this->voucherId = $aCart['voucher']['voucher_id'];
        }
        ...
        try
        {
            ...
            if( $aCart['voucher']['usedVoucherCode'] )
            {
                $this->_voucherUse($aCart['voucher']['usedVoucherCode']);
            }
    ...
    private function __orderItemData($tradeParams, $cartItem, $shopId)
    {
        ...
        $orderData = [
            ...
            'voucher_discount' => $cartItem['price']['voucher_discount'] ? $cartItem['price']['voucher_discount'] : 0,
            'voucher_subsidy_proportion' => $cartItem['voucher_subsidy_proportion'] ? : 0,
    ...
    private function _chgdata( $tradeParams, $aCart=array() )
    {
        ...
        foreach ($aCart['resultCartData'] as $shopId => $shopCartData )
        {
            ...
            $couponCode = kernel::single('systrade_cart_coupon_redis')->get($tradeParams['user_id'], $shopId);
            //当前订单使用的店铺优惠券 用于记录使用店铺优惠券日志
            if( $shopCartData['cartCount']['total_coupon_discount'] > 0 && $couponCode )
            {
                $this->cartUseCoupon[$shopId] = $couponCode;
            }
     ...
     */
    protected function _voucherUse($voucherCode)
    {
        $data = array(
            'user_id' => $this->userId,
            'voucher_code' => $voucherCode,
            'tid' => implode(',',$this->getTids())
        );

        if( !app::get('systrade')->rpcCall('user.voucher.used', $data) )
        {
            throw new \LogicException(app::get('systrade')->_('购物券使用失败'));
        }
