...
class systrade_api_trade_list{
    ...
    public function tradeList($params)
    {
        ...
        $deliveryRow = $params['fields']['extends']['delivery'];
        ...
        //是否需要获取物流参数
        if( $tradeLists && $deliveryRow )
        {
            $deliveryRow = str_append($deliveryRow,'tid');
            $tids = array_column($tradeLists,'tid');
            $deliveryData = app::get('systrade')->rpcCall('delivery.list', ['tid'=>implode(',', $tids)]);
            foreach( $deliveryData as $row )
            {
                $tradeLists[$row['tid']]['delivery']['logi_name'] = $row['logi_name'];
                $tradeLists[$row['tid']]['delivery']['logi_no'] = $row['logi_no'];
                $tradeLists[$row['tid']]['delivery']['corp_code'] = $row['corp_code'];
            }
        }
