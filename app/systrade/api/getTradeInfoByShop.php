...
class systrade_api_getTradeInfoByShop {
    ...
    public function getData($params)
    {
        ...
        $tradeInfo = $this->__changeGiftData($tradeInfo);
        return $tradeInfo;

    }

    private function __changeGiftData($trade)
    {
        $orders = $trade['orders'];
        $newOrders = [];
        foreach($orders as $order)
        {
            ...
            if($order['gift_data'])
            {
                ...
                {
                    $tmpOrder = [];
                    $tmpOrder['oid'] = $order['oid'] . $i;
                    $tmpOrder['order_type'] = 'gift';
                    $tmpOrder['item_type'] = 'gift';
                    $tmpOrder['item_id'] = $gift['item_id'];
                    $tmpOrder['sku_id'] = $gift['sku_id'];
                    $tmpOrder['bn'] = $gift['bn'];
                    $tmpOrder['title'] = $gift['title'];
                    $tmpOrder['num'] = $gift['gift_num'];
                    $tmpOrder['total_fee'] = 0;
                    $tmpOrder['total_weight'] = 0;
                    $tmpOrder['discount_fee'] = 0;
                    $tmpOrder['status'] = $order['status'];
                    $tmpOrder['consign_time'] = $order['consign_time'];
                    $tmpOrder['spec_nature_info'] = $gift['spec_info'];
                    $tmpOrder['payment'] = 0;
                    $tmpOrder['sendnum'] = $order['sendnum'] / $order['num'] * $gift['gift_num'];
                    $tmpOrder['pic_path'] = $gift['image_default_id'];
                    $newOrders[] = $tmpOrder;
                }
            }
        }
