...
class systrade_api_order_status {
    ...
    public function getParams()
    {
        ...
        $return['params'] = array(
            ...
            'refund_fee' => ['type'=>'string', 'valid'=>'', 'default'=>'', 'example'=>'','description'=>'实际退款金额'],
            'total_fee' => ['type'=>'string', 'valid'=>'', 'default'=>'', 'example'=>'','description'=>'商家退款金额'],
        );
    ...
    public function update($params)
    {
        ...
        try
        {
            ...
            {
                //售后退款后需要改变订单的最后修改时间
                $objMdlTrade->update(['modified_time'=>time()],array('tid'=>$params['tid']));
            }
        ...
        event::fire('trade.refund', [$params['tid'], $tradesData['shop_id']]);
    ...
    private function __check($params,&$orderData,&$tradesData,&$ifAll)
    {
        ...
        if($params['aftersales_status'] == "SUCCESS")
        {
            ...
            $orders = $objMdlOrder->getList('shop_id,pay_time,voucher_id,voucher_discount,voucher_subsidy_proportion,tid,oid,status,aftersales_status,user_id,payment,points_fee,gift_data,'.$settlementFields,array('tid'=>$params['tid']));
            ...
            foreach($orders as $key=>$order)
            {
                ...
                if( $order['oid'] != $params['oid'] && $order['aftersales_status'] != 'SUCCESS')
                {
                    $ifAll = false;
                }
        ...
        $tradesData = $objMdlTrade->getRow('shop_id,status,payment,pay_time,post_fee',array('tid'=>$params['tid']));
    ...
    private function __settlement($tradeData, $orderData)
    {
        ...
        kernel::single('sysclearing_vouchersubsidy')->createSubsidyDetail($data,'2');
