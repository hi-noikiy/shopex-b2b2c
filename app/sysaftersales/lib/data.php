...
class sysaftersales_data {
    ...
    public function getAftersalesInfo($fields, $filter)
    {
        ...
        if(in_array($aftersalesInfo['status'],[1,2]) && in_array($aftersalesInfo['progress'],[8,7]))
        {
            ...
            $refunds = $objMdlRefunds->getRow('status,total_price,refund_fee,hongbao_fee',$params);
            $refunds['refund_money'] = ecmath::number_minus(array($refunds['refund_fee'],$refunds['hongbao_fee']));
    ...
    public function getAftersalesList($fields, $filter, $page=1, $limit, $orderBy="created_time desc")
    {
        ...
        $aftersalesList = $this->objMdlAftersales->getList($fields['rows'], $filter, $pageData['offset'], $pageData['limit'], $orderBy);
