...
class topshop_ctl_sku extends topshop_controller{
    ...
    public function getSpecProps()
    {
        ...
        if($itemId)
        {
            ...
            foreach ($result['selection_spec'] as $selectionspec) {
                foreach ($selectionspec['option'] as $key => $value) {
                    $pagedata['specInfo'][$value['spec_value_id']] = $value['spec_value'];
                }
            }
        }
        else
    ...
    public function getParams()
    {
        ...
        if($itemId)
        {
            ...
            if($itemInfo['params'])
            {
                foreach($itemInfo['params'] as $k=>$v)
                {
                    foreach ($v as $k1 => $v1)
                    {
                        $catInfo[$catId]['params'][$k][$k1] = $v1;
                    }
                }
            }
        }
    ...
    {
        $itemInfo = app::get('topshop')->rpcCall('item.get',array('item_id'=>$itemId,'fields'=>'item_id,spec_desc'));
        ...
        if( $itemInfo['spec_desc'] && is_array( $itemInfo['spec_desc'] ) )
        {
            ...
            foreach( $itemInfo['spec_desc'] as $specId => $spec )
            {
                ...
                {
                    $itemInfo['spec'][$specId]['option'][$pSpecId] = $specValue ;
                }
            }
        }
    ...
    private function _select_spec($paramsSpec){
        ...
        {
            $specGoodsImagesArr = app::get('image')->model('images')->getList('id,s_url,url',array('image_id'=>$this->pagedata['goods_spec_images']));
