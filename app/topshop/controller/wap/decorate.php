...
class topshop_ctl_wap_decorate extends topshop_controller {
    ...
    public function index()
    {
        ...
        //判断是否开启新店铺装修
        $pagedata['topshopNewSetup']= app::get('sysconf')->getConf('topshop.firstSetup');
        $pagedata['openNewdecorate'] = redis::scene('shopDecorate')->hget('wapdecorate_status','shop_'.$this->shopId);
    ...
    public function addTags()
    {
        ...
        $this->contentHeaderTitle = app::get('topshop')->_('店铺装修>'.$title);
        if (view::exists("topshop/shop/wap/widgets/{$widgetsName}/add.html"))
        {
            return $this->page("topshop/shop/wap/widgets/{$widgetsName}/add.html", $pagedata);
        }
        else
        {
            return redirect::action('topshop_ctl_wap_decorate@index');
        }
    }
    ...
    //开启新wap店铺装修
    public function ajaxSaveNewDecorate(){
        $status = input::get('status');
        if($status == 'close'){
            $url = url::action('topshop_ctl_wap_decorate@index');
        }elseif($status == 'open'){
            $url = url::action('topshop_ctl_shop_decorate@index');
        }
        $wapstatus = redis::scene('shopDecorate')->hget('appdecorate_status','shop_'.$this->shopId);

        try{
            if(!$wapstatus || $wapstatus=='close' || $status =='open'){
                redis::scene('shopDecorate')->hset('wapdecorate_status','shop_'.$this->shopId,$status);
                redis::scene('shopDecorate')->hset('appdecorate_status','shop_'.$this->shopId,$status);
            }
            if($status =='close' && $wapstatus =='open'){
                redis::scene('shopDecorate')->hset('appdecorate_status','shop_'.$this->shopId,$status);
            }
